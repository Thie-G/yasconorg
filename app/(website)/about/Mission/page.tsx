    ...
    <main className="min-h-screen bg-white mt-20">
      <section className="bg-[#1a2e1a] text-white py-16 px-4 text-center">
        ...
        <p className="text-base text-white/80 max-w-2xl mx-auto leading-relaxed">
          We believe the future of our planet lies in the hands of today's youth.
          At the heart of our work is a commitment to inspire, educate, and empower
          young people to become active stewards of the environment.
        </p>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {profiles.map((profile, idx) => (
            <div key={idx} className="rounded-xl overflow-hidden shadow-lg border border-gray-100 hover:shadow-xl transition-shadow bg-white">
              <div className="relative h-52 w-full">
                <Image src={profile.image} alt={profile.title} fill className="object-cover" />
                <div className="absolute inset-0 bg-gradient-to-t from-[#1a2e1a]/80 to-transparent" />
                <div className="absolute bottom-4 left-4">
                  <span className="text-white font-bold text-lg tracking-wide">{profile.title}</span>
                </div>
              </div>
