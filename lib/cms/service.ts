...
export async function getVacancies(
  opts: { status?: string; region?: string; limit?: number } = {}
) {
  const prisma = getPrismaClient();
  const where: Record<string, unknown> = {};
  if (opts.status && opts.status !== "all") where.status = opts.status;
  if (opts.region && opts.region !== "all") where.region = opts.region;
  return prisma.cmsVacancy.findMany({
    where,
...
export async function getVacancyById(id: number) {
  const prisma = getPrismaClient();
  return prisma.cmsVacancy.findUnique({
    where: { id },
    include: { createdBy: { select: { name: true } } },
  });
}
...
export async function updateVacancy(
  ...
  const prisma = getPrismaClient();
  const existing = await prisma.cmsVacancy.findUnique({ where: { id } });
  const wasPublished = existing?.status !== "published" && data.status === "published";
  return prisma.cmsVacancy.update({
    where: { id },
    data: {
      ...data,
      deadline: data.deadline ? new Date(data.deadline) : null,
      publishedAt: wasPublished ? new Date() : data.publishedAt ? new Date(data.publishedAt) : undefined,
      updatedAt: new Date(),
    },
  });
}
...
export async function deleteVacancy(id: number) {
  const prisma = getPrismaClient();
  await prisma.cmsVacancy.delete({ where: { id } });
}
